...
preference_treatment_scores AS (
    SELECT
        ...
        1 - EXP(-COALESCE(SUM(ca.category_weight), 0)) AS category_multiplier,
        1 - EXP(-COALESCE(SUM(ga.genre_weight), 0))    AS genre_multiplier,
        1 - EXP(-COALESCE(SUM(ta.tag_weight), 0))      AS tag_multiplier,
