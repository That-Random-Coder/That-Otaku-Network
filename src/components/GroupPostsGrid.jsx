...
export default function GroupPostsGrid({ groupId, accent = {} }) {
  ...
  useEffect(() => {
    ...
    const fetchPage = async (p) => {
      ...
      try {
        ...
        try {
          console.log('[GroupPostsGrid] fetching URL:', url)
          console.log('[GroupPostsGrid] authPresent:', !!auth, 'headers:', headers)
        } catch (e) {}
        const res = await fetch(url, { signal: controller.signal, headers })
        const body = await res.json().catch(() => ({}))
        console.log('[GroupPostsGrid] fetch result status:', res.status)
        if (!res.ok) {
        ...
        console.log('[GroupPostsGrid] fetched items:', arr.length, 'page:', number, 'totalPages:', totalPages)
        const more = totalPages > 0 ? (number + 1 < Math.max(1, totalPages)) : (arr.length >= BATCH_SIZE)
        setHasMore(more)
      } catch (e) {
        if (e.name !== 'AbortError') {
          console.error('fetch group posts error', e)
    ...
    if (groupId) fetchPage(page)
    return () => controller.abort()
  }, [page, groupId, currentUserId])
